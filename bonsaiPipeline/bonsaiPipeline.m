...
% figure()
% line(Time,Trigger,'LineWidth',1.2)
% ylim([-0.5,8.5])
...
% for i = 1:1
%     tbl_raw(8,k(isiLarge_pruned(i))+1)=0; % set some elements to 0
% end
k_pruned = find(tbl_raw(8,2:end));
...
for i = 1:numel(k_pruned) % for all k events
    tbl_raw(8,k_pruned(i)+1)=tttable(i,2); % replace the cell with the ith event with the type specified in the arduino list of tones
end
cd 'Z:\Lab Member Folders\Margaret Swerdloff\EEG-gait\EEG\Matlab_data\Troubleshooting'
