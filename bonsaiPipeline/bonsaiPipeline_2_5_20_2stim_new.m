...
% files_all = strcat(sub,'*.dsi');
% Files_all = dir(files_all); % choose 1, 2, or 4 for the order of the butterworth filter
% 
% for k=1:length(Files_all) 
%     prompt = strcat('what condition is this: --- ',Files_all(k).name,' --- ');
%     Files_all(k).condition = input(prompt,'s');
% end
% 
% for k = 1:length(Files_all)
...
%     setNum = k;
%     
%     %         prompt = 'enter the desired directory: ';
%     %         projectdir = input(prompt,'s');
%     %         if isempty(projectdir)
%     %             projectdir = 'Z:\Lab Member Folders\Margaret Swerdloff\EEG_gait\EEG\Matlab_data\Pilot2\S014\sit\C';
%     %         end
%     %         sprintf('Make sure folders exist and that the correct directory is filled out')
%     %         pause
%     
%     session = Files(k).session;
%     projectdir = strcat(location2,'\',session);
%     
    cd 'Z:\Lab Member Folders\Margaret Swerdloff\EEG_gait\EEG\DSI_data\StroopAudio'
%     
%     prompt = 'enter the desired .dsi file: ';
%     filename_only_dsi = input(prompt,'s');
    
%     filename_only = strcat(filename_only_dsi,'.dsi');
filename = 'Maggie_stroopStimuli_v2_unRand_adj_allEasy_v3_AB02.dsi';
    ...
    %filename_tones = 'S014_sit_walk_stand_B.txt';
    ...
    setNameShort = strcat(sub,'_',Files(k).condition,'_',Files(k).session);
    ...
    findpeaks(tone_data)
    xlim([tones_start-2000,tones_start+2000])
    ylim([-0.1,10.1])
    prompt1 = 'enter 1 to continue): ';
    go = input(prompt1);
    xlim([tones_end-2000,tones_end+2000])
    prompt1 = 'enter 1 to continue): ';
    go = input(prompt1);
    ...
    cd(loc_sub)
    save(filename_info,'tones_start','tones_end', '-mat');
    
    % newData1 = load('-mat', filename_info);
    % tones_start = newData1.tones_start;
    % tones_end = newData1.tones_end;
    %% prob_checkR
    %%
    ...
    cd 'Z:\Lab Member Folders\Margaret Swerdloff\EEG_gait\Arduino Tone Lists\All';
    %cd 'D:\Other\transfer'
    ...
    % % % % % tonesAndTimes_struct = open('S003_sit_tonesAndTimes_p3.mat');
    % % % % % tonesAndTimes = tonesAndTimes_struct.tonesAndTimes_new;
    ...
    %text(timeArd,timeline2a,tonenumbers)tonesAndTimes
    xlim([(tones_start/300)-3,(tones_start/300)+15])
    ylim([7.975,8.01])
    ...
    % tbl_raw(8,:) = stim;
    % cd 'Z:\Lab Member Folders\Margaret Swerdloff\EEG_gait\EEG\Matlab_data\Troubleshooting3_05Hz_30Hz'
    % %cd 'C:\Users\maggi\Google Drive\Shared Drives\Northwestern\LeviHargroveLab\pipelinematlabfiles\Troubleshooting3_05Hz_30Hz'
...
%     tbl_filt_a_b_2_tr(8,:) = stim;
    %% Plot data
    cd('Z:\Lab Member Folders\Margaret Swerdloff\EEG_gait\EEG\Matlab_data\StroopAudio\Pilot')
    ...
    % end
    setNameShort='Maggie_stroopStimuli_v2_unRand_adj_allEasy_v3_AB02_raw';
