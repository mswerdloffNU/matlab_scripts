...
filename_only = 'S005_sit.dsi';
filename_duration = strrep(filename_only,'.dsi','_duration_raw.csv');
filename = strrep(filename_only,'.dsi','_raw.csv');
...
if p > 0
    for i = 1:numel(k_k2)-1 % for all k events
...
% pause
% xlim([555,585]) % zoom in on random spot on x-axis
% ylim([7.97,8.1]) 
