...
filename_alone = 'Maggie_stroopStimuli_v2_unRand_adj_allEasy_v3_AB01_duration_raw.csv'; %'MaggieStroopAudio1_0001_duration_raw.csv';
...
stroop_data = 'AB01_Test3unRand_adj_Easy_v3.mat'; %'AB00_Test1 - MMS data.mat';
...
for jj = 1:2
    for ii = 1:length(data.Prov)
        datecell(ii,:,1) = data.Prov{1,ii}.timeStart;
        datecell(ii,:,2) = data.Prov{1,ii}.timeEnd;
        td(ii,1,jj) = duration(datecell(ii,4:6,jj)); %,'inputformat','[yyyy-MM-dd HH:mm:ss.SSS');
    end
end

stroopDurations = milliseconds(td(:,:,2)-td(:,:,1));
stroopStarts(:,1) = milliseconds(td(:,:,2)-(duration(data.Easy{1,1}.timeStart(4:6))));

%% find index of strooop starts and stops

wordOn = find(word==1);
for ii = 1:length(stroopDurations)
    idxSStart(ii) = wordOn(1)+(300*stroopStarts(ii)/1000)-450;
    idxSEnd(ii) = idxSStart(ii)+(300*stroopDurations(ii)/1000);
    idxTStart(ii) = idxSStart(ii)+(300*data.timingError{ii,1});
end
...
for kk = 1:length(idxSStart)
    for ii = 1:length(idxWordsOn_only)
        [nearestProvTones(ii,kk), idxNearestProvTones(ii,kk)] = min(abs(idxSStart(kk)-idxWordsOn_only(ii)));
    end
end

[nearestProvTone idxNearestProvTone] = min(nearestProvTones);

for ii = 1:length(idxSStart)
...
ylabel('word')
hold on 
plot(idxSStart,ones(1,20),'m+') % when the Prov word should have started
...
%%
toneToToneDelay = idxTonesOn_only(2:end)-idxTonesOn_only(1:end-1); %(blue star to blue star)
...
wordToWordDelay_sub(wordToWordDelay_sub > 9000) = 0; %
wordToWordDelay_sub(wordToWordDelay_sub < 872) = 0; %
...
for ii = 1:length(idxTonesOn_only)
    ToneLabelCat(idxTonesOn_only(ii)) = 5; % non-stroop non-target (blue star)
end
for ii = 1:length(nearestToneNonProv)
    ToneLabelCat(nearestToneNonProv(ii)) = 6; % stroop non-target (cyan star)
end
% for ii = 1:length(audioTarget)
%     ToneLabelCat(audioTarget(ii)) = 7; % non-stroop target (magenta circle)
% end
for ii = 1:length(idxWordsOn_only)
