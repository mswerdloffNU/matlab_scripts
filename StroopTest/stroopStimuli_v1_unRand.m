...
if initiationBox == 1
    ...
    dlgTitle = 'Test Data';
    numLines = 1;
    defs = {'AB00','Test 2 UnRand','Z:\Lab Member Folders\Margaret Swerdloff\EEG_gait\StroopTestResults\Troubleshooting\'};
...
for n = 1:ntotal % must be a multiple of 4
    ...
    if timing == 1 % provTime_prior
        ...
        nextTone(:,:,n) = make_wav_file_MMS_unRand(numBase,3)
...
%             nextTime(ii) = (randi([625 1376])*.001);
            nextTime(ii) = (randi([1000 1000])*.001);
            pause(nextTime(ii))
            startTime = tic;
            make_wav_file_MMS_unRand(1,4)
        ...
        pause(1)
        make_wav_file_MMS_unRand(1,1) % proverbial tone
        startProv(n) = tic;
        if provTime(n) ~= 0
