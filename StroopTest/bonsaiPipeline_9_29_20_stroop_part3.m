...
cd(filePath)
Files_v5 = dir('*Maggie*v5*_ICA_Num_*'); % choose 1, 2, or 4 for the order of the butterworth filter
Files_v6 = dir('*Maggie*v6*_ICA_Num_*'); % choose 1, 2, or 4 for the order of the butterworth filter
%%
for f = 1:2
    if f == 1
        Files=Files_v5;
        setName = 'stroop_v5_S015678M1_AR_eq';
    elseif f == 2
        Files=Files_v6;
        setName = 'stroop_v6_hard_S015678M1_AR_eq';
    end
    setName_all='stroop_v5_v6_S015678M1_AR_eq';
    for ii = 1:8:(8*numel(subs))
        filename=Files(ii).name % file to be converted
        if strfind(filename,'Maggie_day1')
            for k=1:8
                Files(k).sub = 'Maggie_day1';
            end
        elseif strfind(filename, 'S015')
            for k=1:8
                Files(k).sub = 'S015';
            end
