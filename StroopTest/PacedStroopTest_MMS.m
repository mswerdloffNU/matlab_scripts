...
%%
for i = 1:10
    secondsFigureChange_all(i) = randi([625 1376])*.001;
end

sfc = secondsFigureChange_all

%% Arg Check and Global Specs definitions
totalRunTime    = 10;   % This is the total time for the test. Based on 
...
% data dir setting
global dataDirPath;
dataDirPath = ('Z:\Lab Member Folders\Margaret Swerdloff\EEG_gait\StroopTestResults\Troubleshooting\');
...
prompt = {'Click OK to start','Enter Phase Number:','Location to save data:'};
dlgTitle = 'Test Data';
numLines = 1;
defs = {'xx','yyyy','Z:\Lab Member Folders\Margaret Swerdloff\StroopTestResults\Troubleshooting\'};
answers = inputdlg(prompt,dlgTitle,numLines,defs);
...
timerObj = runStroopTestWtimerWresults_MMS(totalRunTime,sPortObj,Congruence,FigHandle,sfc); 
% for n = 1:numel(sfc)
%     timerObj = runStroopTestWtimerWresults_MMS(totalRunTime,sPortObj,Congruence,FigHandle,sfc(n));       % Get timer obj which starts the Stroop Test UI
% end
