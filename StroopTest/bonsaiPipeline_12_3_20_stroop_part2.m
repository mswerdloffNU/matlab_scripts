...
for i = 1:numel(subs)
    ...
    for k=1:length(Files_sub)
        ...
        if strfind(filename, 'pt1')
            Files_sub(k).session = 'pt1';
        elseif strfind(filename, 'pt2')
            Files_sub(k).session = 'pt2';
        end
    end
     ...
     EEG = pop_rejepoch( EEG, [icarej_eq(:,2)] ,0);
        ...
        if strcmp(sub,'S015')==0;
            ERP = pop_ploterps( ERP, [ 1 2 3 4 5 6 7 8],  1:7 , 'AutoYlim', 'on', 'Axsize', [ 0.05 0.08], 'BinNum', 'on', 'Blc', 'pre', 'Box', [ 3 3], 'ChLabel',...
                'on', 'FontSizeChan',  10, 'FontSizeLeg',  12, 'FontSizeTicks',  10, 'LegPos', 'bottom', 'Linespec', {'r-' , 'g-' ,'b-', 'c-','r--','g--','b--','c--'}, ...
