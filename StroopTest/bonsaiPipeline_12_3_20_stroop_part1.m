...
for ii = 1:numel(subs)
    ...
    %% arduino generator
    if n == 1
        level = 'v5'; %'v9_mix_pt1'; %'v5_day5';
        filename = 'Sub_v5_stroopStimuli.dsi'; %'Sub_v9_mix_pt1_stroopStimuli.dsi';
    elseif n == 2
        level = 'v6_hard'; %'v9_mix_pt2'; %'v6_hard_day5';
        filename = 'Sub_v6_hard_stroopStimuli.dsi'; %'Sub_v9_mix_pt2_stroopStimuli.dsi';
    end
    filename_location = 'Z:\Lab Member Folders\Margaret Swerdloff\EEG_gait\EEG\DSI_data\All\';
    filename_only = strrep(filename,'Sub',sub);
    if strcmp(level,'v9_mix_pt1') == 1 && strcmp(sub,'Maggie') == 1
        filename_duration1 = strrep(filename_only,'.dsi','_duration_raw_edit.csv');
    else
    ...
    location = strrep(filename_only,'.dsi','_raw.csv'); % for saving only
    ...
    fnameEq =strrep(filename_only,'stroopStimuli.dsi', 'ToneLabelCat.txt');
