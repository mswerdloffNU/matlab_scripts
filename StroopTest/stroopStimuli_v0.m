...
clearvars
close all
ntotal = 4*30; % must be a multiple of 4
sprintf('start')
...
initiationBox = 0; % set to 1 to turn on
...
if initiationBox == 1
    ...
    dlgTitle = 'Test Data';
    numLines = 1;
    defs = {'AB00','Test 1','Z:\Lab Member Folders\Margaret Swerdloff\EEG_gait\StroopTestResults\Troubleshooting\'};
...
for n = 1:ntotal % must be a multiple of 4
    ...
    if timing == 1 % provTime_prior
        ...
        for ii = 1:numEasy
            ...
            make_wav_file_MMS(1,1)
            actualTime(ii) = toc(startTime);
            nextTime(ii) = stroopTimeEasy+(randi([625 1376])*.001);
            set(stroopFigHdl1,'visible','on')
            % Figure Modification starts for next cycle
        ...
        pause(1)
        make_wav_file_MMS(1,4) % proverbial tone
        startProv(n) = tic;
        if provTime(n) ~= 0
        ...
        for ii = ii+1:ii+numHard
            ...
            make_wav_file_MMS(1,1)
            actualTime(ii) = toc(startTime);
            nextTime(ii) = stroopTimeHard+(randi([625 1376])*.001);
            set(stroopFigHdl3,'visible','on')
            % Figure Modification starts for next cycle
