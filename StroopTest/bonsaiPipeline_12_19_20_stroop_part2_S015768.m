...
% AR = 1;
% AR_eq_outRem = 1; % goes from Pilot2 folder to ner2021/AR_eq_outRem
% AR_eq = 2; % (old) from Pilot2 to AR_eq
% AR = 3; % from ner2021 to ner2021/AR folder
%
%% DO NOT UNCOMMENT. USE ONLY TO REMOVE OUTLIERS
% tbl_filt_a_b_2_tr(8,281176)=9; %S016 v5
% tbl_filt_a_b_2_tr(8,545372)=9; %S016 v5
% tbl_filt_a_b_2_tr(8,102011)=9; %S018 v6
% tbl_filt_a_b_2_tr(8,130694)=9; %S018 v6
%%
for i = 1:numel(subs)
    clearvars -except subs i
    sub = subs{i}
    loc_sub = strcat('Z:\Lab Member Folders\Margaret Swerdloff\EEG_gait\EEG\Matlab_data\StroopAudio\Pilot2\ner2021\AR_outRem\',sub);
    filePath=loc_sub;
    %% select file
    cd(loc_sub)
    ...
    for k=1:length(Files)
        ...
        setNum = k;
%         if strfind(filename,'v5') > 0
%             if strfind(filename,'S015') > 0
%                 sprintf('hello!')
%             end
%         end
        %%
        ...
        %% move to EEGLAB
        cd 'C:\Users\mswerdloff\eeglab\eeglab2020_0'
        eeglab
        ...
        EEG = eeg_checkset( EEG );
        EEG = pop_editset(EEG, 'chanlocs', 'C:\Users\mswerdloff\eeglab\eeglab2020_0\chanlocsDSI7.ced');
        [ALLEEG EEG] = eeg_store(ALLEEG, EEG, CURRENTSET);
...
%     ALLEEG(5).EVENTLIST.eventinfo = icarej_eq(:,2);
    %% actually reject epochs
