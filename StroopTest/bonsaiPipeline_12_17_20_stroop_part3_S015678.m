...
if old == 1 % Since Maggie files are also located in that folder
    Files_v5(1:8) = [];
    Files_v6(1:8) = [];
end
...
for f = 1:2
    if f == 1
        Files=Files_v5;
        setName = 'stroop_v5_S015678_AR';
    elseif f == 2
        Files=Files_v6;
        setName = 'stroop_v6_hard_S015678_AR';
    end
    setName_all= 'stroop_v5_v6_S015678_AR';
    for ii = 1:length(Files)
        ...
        elseif strfind(filename,'S018')
            ...
            warning('you need to name the next sub')
        end
    end
...
for kk = 1:numel(Files)
    ...
    T_sorted = table2struct(aa_t);
    
    clear stroop_noAR stroop_noAR_pz
    for ii = 1:(8*numel(subs))
%         stroop_noAR = importTarget(Files(ii).name);
        stroop_noAR = importTarget(T_sorted(ii).name);
        stroop_noAR_pz(:,ii) = table2array(stroop_noAR(2,2:end))';
    end
    stroop_noAR_pz(:,8)=NaN; % since there was no StroopCongruent measured for S015v5
...
%             vDiff = v5_all(:,ii)-v5_all(:,ii-4); %easy-hard
            vDiff = v5_all(:,ii)-v6_all(:,ii); %easy-hard
...
if PLOT == 1
    ...
    for kk = 1:4
        hold on
        plot(time,v6_all(:,kk),'LineWidth',2)
    end
    line([0 0], [-40 40]);
    line([-200 800], [0 0]);
    title('Level - HARD')
    ...
    for kk = 1:4
        hold on
        plot(time,v5_all(:,kk)-v6_all(:,kk),'LineWidth',2)
    end
    line([0 0], [-40 40]);
    line([-200 800], [0 0]);
    title('EASY - HARD')
...
colors = [0 0.447 .741;.85 .325 .098;.929 .694 .125;.494 .184 .556;0 0.447 .741;.85 .325 .098;.929 .694 .125;.494 .184 .556];
figure
...
%
clear v_diff_new2
v_diff_new3 = [v5_all-v6_all]';
for kk = 1:4
v_diff_new2(:,kk) = [v5_all(:,kk)-v6_all(:,kk)]';
end
%%
    for kk = 1:8
        % for ii = kk:8:(8*numel(subs))
        ...
        v_diff_grandAvg_sum2(kk,:) = (nansum(v_diff_new2'))/n_adj;
%         v_diff_grandAvg_sum3(kk,:) = (nansum(v_diff_new3'))/n_adj;
    end
%%
%     pause
%% Easy vs Hard for each time group 
...
% frame = [490 540]*sr/1000;
% frame = [400 700]*sr/1000;
frame = [(200+250) (200+500)]*sr/1000; % ADD 200 TO ACCOUNT FOR BASELINE
...
%     frame_sterr_all = [std(frame_avg_1)/sqrt(length(frame_avg_1)) std(frame_avg_2)/sqrt(length(frame_avg_2)) std(frame_avg_3)/sqrt(length(frame_avg_3)) std(frame_avg_4)/sqrt(length(frame_avg_4)) std(frame_avg_5)/sqrt(length(frame_avg_5)) std(frame_avg_6)/sqrt(length(frame_avg_6)) std(frame_avg_7)/sqrt(length(frame_avg_7)) std(frame_avg_8)/sqrt(length(frame_avg_8))];
        ...
        [p_w_diff,h_w_diff] = signrank(frame_avg_3, frame_avg_2);
...
for i = 1:nsub
v5_avg_7(i) = mean(v5_all(frame(1):frame(2),((i-1)*8)+7));
v6_avg_7(i) = mean(v6_all(frame(1):frame(2),((i-1)*8)+7));
end

frame_mean_audio = [mean(v5_avg_7) mean(v6_avg_7)];
frame_sterr_audio = [std(v5_avg_7)/sqrt(length(v5_avg_7)) std(v6_avg_7)/sqrt(length(v6_avg_7))];
[p_audio,h_audio] = signrank(v5_avg_7,v6_avg_7);

figure
errorbarbar(frame_mean_audio,frame_sterr_audio,axes,[.2 .2 .2; .8 .8 .8])
ylabel('Mean Decrease in P3 Potential (µV)')
...
hold on
for kk = 1:4
plot(time,v_diff_grandAvg_sum(kk,:),'LineWidth',2)
hold on
end

%% COMPARE across averaging methods
figure
subplot(1,4,1)
for kk = 1:4
    hold on
    plot(time,v_diff_new2(:,kk),'LineWidth',2)
end
line([0 0], [-40 40]);
line([-200 800], [0 0]);
subplot(1,4,2)
...
for kk = 1:4
    hold on
    plot(time,v_diff_grandAvg_sum2(kk,:),'LineWidth',2)
end
line([0 0], [-40 40]);
line([-200 800], [0 0]);
subplot(1,4,4)
for kk = 1:4
    hold on
    plot(time,v_diff_new3(kk,:),'LineWidth',2)
end
line([0 0], [-40 40]);
line([-200 800], [0 0]);
...
line([-200 800], [0 0]);
title('S018')
legend('Pre-Stroop Onset','At Stroop Onset','After Stroop Onset','After Stroop Onset x 2')

%% COMPARE across timings
figure
subplot(1,4,1)
for kk = [1,9,17,25]
    hold on
    plot(time,v_diff_new3(kk,:),'LineWidth',1)
    plot(time,mean(v_diff_new3([1,9,17,25],:)),'k','LineWidth',2)
end
line([0 0], [-40 40]);
line([-200 800], [0 0]);
title('Pre-Stroop Onset')
...
for kk =  [2,10,18,26]
    hold on
    plot(time,v_diff_new3(kk,:),'LineWidth',1)
    plot(time,mean(v_diff_new3([2,10,18,26],:)),'k','LineWidth',2)
end
line([0 0], [-40 40]);
line([-200 800], [0 0]);
title('At Stroop Onset')
...
for kk = [3,11,19,27]
    hold on
    plot(time,v_diff_new3(kk,:),'LineWidth',1)
    plot(time,mean(v_diff_new3([3,11,19,27],:)),'k','LineWidth',2)
end
line([0 0], [-40 40]);
line([-200 800], [0 0]);
title('After Stroop Onset')
...
for kk = [4,12,20,28]
    hold on
    plot(time,v_diff_new3(kk,:),'LineWidth',1)
    plot(time,mean(v_diff_new3([4,12,20,28],:)),'k','LineWidth',2)
end
line([0 0], [-40 40]);
line([-200 800], [0 0]);
title('After Stroop Onset x 2')
legend('S015','S016','S017','S018')
%% check check
timingA = (v_diff_new3([1,9,17,25],:));
timingAm = mean(timingA);
figure
plot(time,timingAm-mean(v_diff_new3([1,9,17,25],:)))
%%
clear y_diff_new
frame = [(250+200) (500+200)]*sr/1000; % ADD 200 TO ACCOUNT FOR BASELINE

for kk = 1:4
    y_diff_new(kk) = mean(v_diff_new2(frame(1):frame(2),kk));
end
...
figure
% errorbarbar([y_v5_minusStroop(2); y_v6_minusStroop(2)]',[err_v5_minusStroop(2); err_v6_minusStroop(2)]',axes,[colors])
errorbarbar([y_diff_new],[err_diff],axes,[colors])
ylabel('Mean Decrease in P3 Potential (µV)')
title('RECALCULATED 12/19/2020')

%%
Fsp = 300; % sampling rate in Hz
Fn = Fsp/2; % Nyquist frequency
[b,a] = butter(2,20/Fn,'low');

for kk = 1:7
v5_grandAvg_sum_filt(kk,:) = filtfilt(b,a,v5_grandAvg_sum(kk,:));
v6_grandAvg_sum_filt(kk,:) = filtfilt(b,a,v6_grandAvg_sum(kk,:));
v_diff_grandAvg_sum_filt(kk,:) = filtfilt(b,a,v_diff_grandAvg_sum(kk,:));
end
%% Easy vs Hard for each time group 
colors = [0 0.447 .741;.85 .325 .098;.929 .694 .125;.494 .184 .556];
figure
for kk = 1:4
    subplot(1,4,kk)
    plot(time,v5_grandAvg_sum_filt(kk,:),'k-.','LineWidth',1)
    hold on
    plot(time,v6_grandAvg_sum_filt(kk,:),'Color',[.5 .5 .5],'LineWidth',1)
    
    plot(time,v_diff_grandAvg_sum_filt(kk,:),'Color',colors(kk,:),'LineWidth',3)
...
%         title('After Stroop Onset x 2')
%     end
    %legend('Easy','Difficult','Easy-Difficult')
   
end
fnm = sprintf('erp_diff_filt.fig');
%cd('Z:\Lab Member folders\Margaret Swerdloff\EEG_gait\EEG\Matlab_data\StroopAudio\Pilot2\ERP_plots');
%     savefig(fnm)
...
figure
plot(time,v5_grandAvg_sum_filt(7,:),'k-.','LineWidth',2)
hold on
plot(time,v6_grandAvg_sum_filt(7,:),'Color',[.5 .5 .5],'LineWidth',2)
line([0 0], [-10 20]);
line([-200 800], [0 0]);
grid off
% xlabel('Time relative to stimuli (ms)')
...
%%
figure
fH = gcf; colormap(jet(2));
errorbarbar(frame_mean_audio,frame_sterr_audio,axes)
ylabel('Mean Decrease in P3 Potential (µV)')
applyhatch_plusC(fH, '\-', 'kk');
% applyhatch_plusC(fH, '\-x.', 0, [1 0], jet(2));
...
%     end
    ...
    clear stroop_noAR stroop_noAR_pz
    for ii = 1:length(T_sorted)
        %         stroop_noAR = importTarget(Files(ii).name);
        stroop_noAR = importTarget(T_sorted(ii,1).name);
        stroop_noAR_pz(:,ii) = table2array(stroop_noAR(2,2:end))';
    end
...
for version = 1:2
    ...
    frame_avg_all = [frame_avg_1; frame_avg_2; frame_avg_3; frame_avg_4; frame_avg_5; frame_avg_6; frame_avg_7; frame_avg_8]';
    frame_mean_all = [nanmean(frame_avg_1) nanmean(frame_avg_2) nanmean(frame_avg_3) nanmean(frame_avg_4) nanmean(frame_avg_5) mean(frame_avg_6) nanmean(frame_avg_7) nanmean(frame_avg_8)];
    frame_sterr_all = [nanstd(frame_avg_1)/sqrt(length(frame_avg_1)) nanstd(frame_avg_2)/sqrt(length(frame_avg_2)) nanstd(frame_avg_3)/sqrt(length(frame_avg_3)) nanstd(frame_avg_4)/sqrt(length(frame_avg_4)) nanstd(frame_avg_5)/sqrt(length(frame_avg_5)) nanstd(frame_avg_6)/sqrt(length(frame_avg_6)) nanstd(frame_avg_7)/sqrt(length(frame_avg_7)) nanstd(frame_avg_8)/sqrt(length(frame_avg_8))];
    %     frame_avg_all = [frame_avg_1; frame_avg_2; frame_avg_3; frame_avg_4]';
    %     frame_mean_all = [mean(frame_avg_1) mean(frame_avg_2) mean(frame_avg_3) mean(frame_avg_4)];
    %     frame_sterr_all = [std(frame_avg_1)/sqrt(length(frame_avg_1)) std(frame_avg_2)/sqrt(length(frame_avg_2)) std(frame_avg_3)/sqrt(length(frame_avg_3)) std(frame_avg_4)/sqrt(length(frame_avg_4))];
    if version == 1
...
colors = [0 0.447 .741;.85 .325 .098;.929 .694 .125;.494 .184 .556];
y_diff = y_v5-y_v6;
err_y_diff = err_v5-err_v6;
figure
errorbarbar(y_diff(1:4),err_y_diff(1:4),axes,colors)
