...
function [nextTone, nextTone_val, timing] = make_wav_file_MMS_unRand_adj_v10(number,tone,timing,pre_tone)
...
% folder = pwd;
folder = 'Z:\Lab Member folders\Margaret Swerdloff\EEG_gait\EEG\Matlab_data\StroopAudio\Pilot2\Maggie';
...
audiowrite(fullFileName1, y1, 8000);
audiowrite(fullFileName2, y2, 8000);
...
elseif b == 3
    for i = 1:round((number-1)/2)
        ...
        %             nextTone(i) = 0.5+(randi([625 1376])*.001);
        nextTone_val(i) = 2;
        nextTone(i) = (0.5+(randi([1200 1300])*.001))/2; %(0.5+(randi([1288 1288])*.001))/2;
        if pre_tone == 1
            pause(nextTone(i))
        end
    end
    ...
    nextTone(i) = (0.5+(randi([1200 1300])*.001))/2; %(0.5+(randi([1288 1288])*.001))/2;
    pause(nextTone(i))
    for i = 1:(number-round((number-1)/2)-1)
        if pre_tone == 1
            PlaySoundFile(folder, baseFileName2);
        end
        %             nextTone(i) = 0.5+(randi([625 1376])*.001);
...
elseif b == 4
    for i = 1:number
        ...
        nextTone_val(i) = 2;
        nextTone(i) = 0.5+(randi([200 300])*.001); %0.5+(randi([250 250])*.001);
        pause(nextTone(i))
    end
elseif b == 5
    for i = 1:number
