...
TargetsAccepted_allSubs = array2table(targetsAccepted_all,'VariableNames',{'Sit','Stand','Walk','Min'},'RowNames',{'S003','S005','S006','S007','S008','S009','S010','S012','S013','S014'});
...
TotalAccepted_allSubs = array2table(totalAccepted_all,'VariableNames',{'Sit','Stand','Walk'},'RowNames',{'S003','S005','S006','S007','S008','S009','S010','S012','S013','S014'});
...
x_all = table2array(RejectedTrials_S003);
...
shouldBeRej = 90-numel(list_rej)-targetsNewNum; % number of targets that need to be rejected
reject = randsample(n_accepted_total,shouldBeRej); % list of target trials that should be rejected
...
for i = 1:numel(list_accept)
    if (newlist(i) == 0) && (codes_sit(i) == 1) && (list_accept(i) < 302)
        rejectedTrials_new(i,1)=4;
    elseif (newlist(i) == 0) && (codes_sit(i) == 1) && (list_accept(i) > 602)
        rejectedTrials_new(i-602,3)=4;
    elseif (newlist(i) == 0) && (codes_sit(i) == 1) && (list_accept(i) < 603) && (list_accept(i) > 301)
        rejectedTrials_new(i-301,2)=4;
    end
end

newlyRej = numel(find(rejectedTrials_new(:,1:3)==4)) + numel(list_rej) - 3; % minus 3 since there are already 3 at the beginning
if newlyRej ~= sum([shouldBeRej;numel(list_rej)])
    sprintf('error changing number of rejected trials')
    pause
else 
    sprintf('good sit!')
end
...
for i = 1:numel(list_accept)
    if (newlist(i) == 0) && (codes_sit(i) == 1) && (list_accept(i) < 302)
        rejectedTrials_new(b,4)=4;
    elseif (newlist(i) == 0) && (codes_sit(i) == 1) && (list_accept(i) > 602)
        rejectedTrials_new(i-602,6)=4;
    elseif (newlist(i) == 0) && (codes_sit(i) == 1) && (list_accept(i) < 603) && (list_accept(i) > 301)
        rejectedTrials_new(i-301,5)=4;
    end
end

newlyRej = numel(find(rejectedTrials_new(:,4:6)==4)) + numel(list_rej) - 3;
if newlyRej ~= sum([shouldBeRej;numel(list_rej)])
    sprintf('error changing number of rejected trials')
    pause
else 
...
for i = 1:numel(list_accept)
    if (newlist(i) == 0) && (codes_sit(i) == 1) && (list_accept(i) < 302)
        rejectedTrials_new(b,7)=4;
    elseif (newlist(i) == 0) && (codes_sit(i) == 1) && (list_accept(i) > 602)
        rejectedTrials_new(i-602,9)=4;
    elseif (newlist(i) == 0) && (codes_sit(i) == 1) && (list_accept(i) < 603) && (list_accept(i) > 301)
        rejectedTrials_new(i-301,8)=4;
    end
end

newlyRej = numel(find(rejectedTrials_new(:,7:9)==4)) + numel(list_rej) - 3;
if newlyRej ~= sum([shouldBeRej;numel(list_rej)])
    sprintf('error changing number of rejected trials')
    pause
else 
...
x_all = table2array(RejectedTrials_S003_new);
clear totalAccepted targetsAccepted
for i = 1:9
    totalAccepted(i) = 300-numel(find(x_all(:,i)));
    targetsAccepted(i) = 31-numel(find(x_all(:,i)==1))-numel(find(x_all(:,i)==4));
end
...
targetsAccepted_walk = sum(targetsAccepted(7:9));
targetsAccepted_all = [targetsAccepted_sit,targetsAccepted_stand,targetsAccepted_walk];
targetsAccepted_all_S003_new = [targetsAccepted_sit,targetsAccepted_stand,targetsAccepted_walk,min(targetsAccepted_all)]
totalAccepted_all_S003_new = [sum(totalAccepted(1:3)),sum(totalAccepted(4:6)),sum(totalAccepted(7:9))];

cd 'Z:\Lab Member Folders\Margaret Swerdloff\EEG-gait\EEG\Matlab_data\Pilot2\S003'
save('rejectedAcceptedTrials_new.mat','RejectedTrials_S003_new','targetsAccepted_all_S003_new','totalAccepted_all_S003_new'); % save list of numbers of targets accepted

if targetsAccepted_all_S003_new == ones(1,4)*(targetsNewNum)
    sprintf('good!')
else
