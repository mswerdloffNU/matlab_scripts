%%
subs = {'S003','S005','S006','S007','S008','S009','S010','S012','S013','S014'};
% now, put everything into the original format
for i = 1:numel(subs)
close all
load('Z:\Lab Member Folders\Margaret Swerdloff\EEG-gait\EEG\Matlab_data\Pilot2\codes_allSubs.mat') %codes
sub = subs{i};
...
clear x_all_combined x_all codes_all
...
cd(loc_sit)
Files_sit = dir('*eq10_ICA_Num_Target.txt'); % choose 1, 2, or 4 for the order of the butterworth filter
for k=1:length(Files_sit)
   filename=Files_sit(k).name % file to be converted
...
cd(loc_stand)
Files_stand = dir('*eq10_ICA_Num_Target.txt');
for k=1:length(Files_stand)
   filename=Files_stand(k).name % file to be converted
   if strfind(filename, '_A_')
...
cd(loc_walk)
Files_walk = dir('*eq10_ICA_Num_Target.txt');
for k=1:length(Files_stand)
   filename=Files_walk(k).name % file to be converted
   if strfind(filename, '_A_')
...
% clear normParam_alt normParam_alt_num
...
clear S00X_sit_avg_eq S00X_stand_avg_eq S00X_walk_avg_eq
...
clear -except i S00X_sit_avg_eq S00X_stand_avg_eq S00X_walk_avg_eq subs
end

%% plot grand averages
cd 'Z:\Lab Member Folders\Margaret Swerdloff\EEG-gait\EEG\Matlab_data\Pilot2' % destination folder
